...
data_input <- as.matrix(scRNA_chat@assays$RNA@data) # normalized data matrix
meta = scRNA@meta.data # a dataframe with rownames containing cell mata data
cell.use = rownames(meta)[meta$orig.ident == "BM"] # extract the cell names from disease data
...
mat2 <- matrix(0, nrow = nrow(mat), ncol = ncol(mat), dimnames = dimnames(mat))
mat2[12, ] <- mat[12, ]
netVisual_circle(mat2, vertex.weight = groupSize, weight.scale = T, edge.weight.max = max(mat), title.name = rownames(mat)[12])
...
for (i in 1:length(pathways.show.all)) {
  ...
  ggsave(filename=paste0(pathways.show.all[i], "_L-R_contribution.pdf"), plot=gg, width = 15, height = 10, units = 'in', dpi = 500)
}
```
